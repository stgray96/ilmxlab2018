/* Stewart Gray III
 * August 24th, 2017
 * Gathers depth information from the Microsoft Kinect sensor, and extrapolates a set of points (within the range) to create a point cloud of one person in front of the camera
 * The point cloud is stored as a Unity mesh object, allowing its vertices to be imported by our "PointToGeometry" shader for further processing
 */
...
public class PointCloudGenerator : MonoBehaviour
{
    ...
    [SerializeField]
	//The scale determines the pixel offset when reading the Kinect's depth sensor image. Every 64th pixel (within our threshold) will be accounted for to maintain optimal performance.
    ...
    //Obtain the source for the Kinect's depth sensor 
    private DepthFrameSource depthSource = null;
    //Allows us to read each frame obtained from the depth sensor
    ...
    //Kinectv2 Camera Information. These are hardcoded values obtained from MSDN's Kinect API documentation
	...
	/* When provided with a frame from the Kinect's depth camera, this parses each valid pixel, and stores its corresponding vertex position (after the conversion) into a new mesh object */
	...
	/* Gathers a new frame from the Kinect depth sensor, and updates our mesh accordingly */
    void Update()
    {
        if (depthReader != null)
        {
	...
	/* Utilizes the MSDN provided pre-defined values to convert each two-dimensional pixel from the depth camera into three dimensional space */
	...
	/* Calculates the difference between two pixels within our depth image. If the distance is too great, we can consider it to be a stray pixel, and should NOT add it to our point cloud */
	...
	/* The Kinect must be closed and its depth readers disposed upon quitting the application to allow new frames to be rendered upon restart */
